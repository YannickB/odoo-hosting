...
class ClouderBase(models.Model):
    ...
    def deploy_test(self):
        ...
        if self.application_id.type_id.name == 'drupal' and self.application_id.code == 'wkc':
            ...
            self.execute(ssh, ['drush', 'vset', '--yes', '--exact', 'wikicompare_test_platform', '1'], path=self.service_id.full_localpath_files() + '/sites/' + self.fulldomain())
            if self.poweruser_name and self.poweruser_email:
                self.execute(ssh, ['drush', self.service_id.full_localpath_files() + '/wikicompare.script', '--user=' + self.poweruser_name, 'deploy_demo'], path=self.service_id.full_localpath_files() + '/sites/' + self.fulldomain())
            ssh.close(), sftp.close()
        return res
