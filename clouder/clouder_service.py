...
class ClouderService(models.Model):
    ...
    @api.multi
    def name_get(self):
        return (self.id, self.name + ' [' + self.container_id.name + '_' + self.container_id.server_id.name +  ']')
    ...
    def name_search(self, name='', args=None, operator='ilike', limit=100):
        if not args:
            args = []
        if name:
            ids = self.search(['|',('name','like',name),'|',('container_id.name','like',name),('container_id.server_id.name','like',name)]+ args, limit=limit)
        else:
    ...
    full_localpath_files = lambda self : self.application_id.type_id.localpath_services + '/' + self.name + '/files'
    ...
    def _check_application(self):
        if self.application_id.id != self.container_id.application_id.id:
            raise except_orm(_('Data error!'),
                _("The application of service must be the same than the application of container."))

    @api.one
    @api.constrains('application_id','application_version_id')
    def _check_application_version(self):
        if self.application_id.id != self.application_version_id.application_id.id:
            raise except_orm(_('Data error!'),
                _("The application of application version must be the same than the application of service."))
    ...
    def _check_database(self):
        if not self.database():
            raise except_orm(_('Data error!'),
                _("You need to specify a database in the links of the service " + self.name + " " + self.container_id.fullname()))
    ...
    def _check_option_ids(self):
        for type_option in self.application_id.type_id.option_ids:
            if type_option.type == 'service' and type_option.required:
                ...
                if not test:
                    raise except_orm(_('Data error!'),
                        _("You need to specify a value for the option " + type_option.name + " for the service " + self.name + "."))
    ...
    def _check_link_ids(self):
        for app_link in self.application_id.link_ids:
            if app_link.service and app_link.required:
                ...
                if not test:
                    raise except_orm(_('Data error!'),
                        _("You need to specify a link to " + app_link.name + " for the service " + self.name))
    ...
    def install_subservice(self):
        if not self.sub_service_name or self.sub_service_name == self.name:
            return
        services = self.search([('name','=',self.sub_service_name),('container_id','=', self.container_id.id)])
        services.unlink()
        options = []
        type_ids = self.env['clouder.application.type.option'].search([('apptype_id','=',self.container_id.application_id.type_id.id),('name','=','port')])
        if type_ids:
        ...
        for base in self.base_ids:
            ...
            self = self.with_context(save_comment='Duplicate base into ' + subbase_name)
    ...
    def deploy(self):
        ...
        if self.database_type() != 'mysql':
            ssh, sftp = self.connect(self.database().fullname(), username='postgres')
            self.execute(ssh, ['psql', '-c', '"CREATE USER ' + self.db_user() + ' WITH PASSWORD \'' + self.database_password + '\' CREATEDB;"'])
            ssh.close(), sftp.close()

            ssh, sftp = self.connect(self.container_id.fullname(), username=self.application_id.type_id.system_user)
            self.execute(ssh, ['sed', '-i', '"/:*:' + self.db_user() + ':/d" ~/.pgpass'])
            self.execute(ssh, ['echo "' + self.database_server() + ':5432:*:' + self.db_user() + ':' + self.database_password + '" >> ~/.pgpass'])
            self.execute(ssh, ['chmod', '700', '~/.pgpass'])
            ssh.close(), sftp.close()

        else:
            ssh, sftp = self.connect(self.database().fullname())
            self.execute(ssh, ["mysql -u root -p'" + self.database().root_password() + "' -se \"create user '" + self.db_user() + "' identified by '" + self.database_password + "';\""])
            ssh.close(), sftp.close()
    ...
    def purge(self):
        ...
        if self.database_type() != 'mysql':
            ssh, sftp = self.connect(self.database().fullname(), username='postgres')
            self.execute(ssh, ['psql', '-c', '"DROP USER ' + self.db_user() + ';"'])
            ssh.close(), sftp.close()
            ...
            ssh, sftp = self.connect(self.database().fullname())
            self.execute(ssh, ["mysql -u root -p'" + self.database().root_password + "' -se \"drop user " + self.db_user() + ";\""])
            ssh.close(), sftp.close()
    ...
    def check_files(self):
        services = self.search([('application_version_id', '=', self.application_version_id.id),('container_id.server_id','=',self.container_id.server_id.id)])
        if self in services:
    ...
    def deploy_files(self):
        ...
        if not self.exist(sftp, self.application_version_id.full_hostpath):
            ...
            self.log('sftp get ' + self.application_version_id.full_archivepath_targz + ' ' + tmp)
            sftp_archive.get(self.application_version_id.full_archivepath_targz, tmp)
        ...
        if 'files_from_service' in self.env.context:
            self.execute(ssh, ['cp', '-R', self.application_id.type_id.localpath_services + '/' + self.env.context['files_from_service'] + '/files', self.full_localpath_files])
        elif self.custom_version or not self.application_id.type_id.symlink:
            self.execute(ssh, ['cp', '-R', self.application_version_id.full_localpath, self.full_localpath_files])
        else:
            self.execute(ssh, ['ln', '-s', self.application_version_id.full_localpath, self.full_localpath_files])
...
class ClouderServiceOption(models.Model):
    ...
    def _check_required(self):
        if not self.name.required and not self.value:
            raise except_orm(_('Data error!'),
                _("You need to specify a value for the option " + self.name.name + " for the service " + self.service_id.name + "."))
...
class ClouderServiceLink(models.Model):
    ...
    def control(self):
        if not self.target:
            ...
            return False
        app_links = self.search([('service_id','=',self.service_id.id),('name.code','=', self.target.application_id.code)])
        if not app_links:
            self.log('The target isnt in the application link for service, skipping deploy link')
            return False
