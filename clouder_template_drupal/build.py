...
class clouder_application_version(osv.osv):
    ...
    def build_application(self, cr, uid, vals, context):
        ...
        if vals['apptype_name'] == 'drupal':
            ...
            sftp.put(modules.get_module_path('clouder_drupal') + '/res/wikicompare.script', vals['app_version_full_archivepath'] + '/wikicompare.script')
            sftp.put(modules.get_module_path('clouder_drupal') + '/res/patch/revisioning_postgres.patch', vals['app_version_full_archivepath'] + '/revisioning_postgres.patch')
