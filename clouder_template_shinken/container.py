...
class clouder_container_link(osv.osv):
    ...
    def deploy_link(self, cr, uid, vals, context={}):
        super(clouder_container_link, self).deploy_link(cr, uid, vals, context=context)
        if vals['link_target_app_code'] == 'shinken':
            ...
            sftp.put(modules.get_module_path('clouder_shinken') + '/res/' + file + '.config', vals['container_shinken_configfile'])
    ...
    def purge_link(self, cr, uid, vals, context={}):
        super(clouder_container_link, self).purge_link(cr, uid, vals, context=context)
        if vals['link_target_app_code'] == 'shinken':
            ssh, sftp = execute.connect(vals['link_target_container_fullname'], context=context)
