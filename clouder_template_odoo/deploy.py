...
#         res = super(clouder_container, self).add_links(cr, uid, vals, context=context)
...
class clouder_service(osv.osv):
    ...
    def get_vals(self, cr, uid, id, context=None):
        ...
        vals = super(clouder_service, self).get_vals(cr, uid, id, context=context)
    ...
    def deploy_post_service(self, cr, uid, vals, context):
        super(clouder_service, self).deploy_post_service(cr, uid, vals, context)
...
class clouder_base(osv.osv):
    ...
    def deploy_create_database(self, cr, uid, vals, context=None):
        res = super(clouder_base, self).deploy_create_database(cr, uid, vals, context)
        context.update({'clouder-self': self, 'clouder-cr': cr, 'clouder-uid': uid})
        if vals['apptype_name'] == 'odoo':
            ssh, sftp = execute.connect(vals['container_fullname'], username=vals['apptype_system_user'], context=context)
...
#                execute.execute_local(cmd, context, stdin_arg=stdin)
    ...
    def deploy_build(self, cr, uid, vals, context=None):
        res = super(clouder_base, self).deploy_build(cr, uid, vals, context)
        context.update({'clouder-self': self, 'clouder-cr': cr, 'clouder-uid': uid})
        if vals['apptype_name'] == 'odoo':
    ...
    def deploy_post(self, cr, uid, vals, context=None):
        res = super(clouder_base, self).deploy_post(cr, uid, vals, context)
        context.update({'clouder-self': self, 'clouder-cr': cr, 'clouder-uid': uid})
        if vals['apptype_name'] == 'odoo':
    ...
    def deploy_create_poweruser(self, cr, uid, vals, context=None):
        res = super(clouder_base, self).deploy_create_poweruser(cr, uid, vals, context)
        context.update({'clouder-self': self, 'clouder-cr': cr, 'clouder-uid': uid})
        if vals['apptype_name'] == 'odoo':
    ...
    def deploy_test(self, cr, uid, vals, context=None):
        res = super(clouder_base, self).deploy_test(cr, uid, vals, context)
        context.update({'clouder-self': self, 'clouder-cr': cr, 'clouder-uid': uid})
        if vals['apptype_name'] == 'odoo':
            execute.log("client = erppeek.Client('http://" + vals['server_domain'] + ":" + vals['service_options']['port']['hostport'] + "," + "db=" + vals['base_unique_name_'] + "," + "user=" + vals['apptype_admin_name'] + ", password=" + vals['base_admin_passwd'] + ")", context)
            client = erppeek.Client('http://' + vals['server_domain'] + ':' + vals['service_options']['port']['hostport'], db=vals['base_unique_name_'], user=vals['apptype_admin_name'], password=vals['base_admin_passwd'])
    ...
    #     res = super(clouder_base, self).deploy_prepare_apache(cr, uid, vals, context)
    #     context.update({'clouder-self': self, 'clouder-cr': cr, 'clouder-uid': uid})
    #     if vals['apptype_name'] == 'odoo':
    ...
    def post_reset(self, cr, uid, vals, context=None):
        res = super(clouder_base, self).post_reset(cr, uid, vals, context)
        context.update({'clouder-self': self, 'clouder-cr': cr, 'clouder-uid': uid})
        if vals['apptype_name'] == 'odoo':
            execute.log("client = erppeek.Client('http://" + vals['server_domain'] + ":" + vals['service_options']['port']['hostport'] + "," + "db=" + vals['base_unique_name_'] + "," + "user=" + vals['apptype_admin_name'] + ", password=" + vals['base_admin_passwd'] + ")", context)
            client = erppeek.Client('http://' + vals['server_domain'] + ':' + vals['service_options']['port']['hostport'], db=vals['base_unique_name_'], user=vals['apptype_admin_name'], password=vals['base_admin_passwd'])
    ...
    def update_base(self, cr, uid, vals, context=None):
        res = super(clouder_base, self).update_base(cr, uid, vals, context)
        context.update({'clouder-self': self, 'clouder-cr': cr, 'clouder-uid': uid})
        if vals['apptype_name'] == 'odoo':
            try:
                execute.log("client = erppeek.Client('http://" + vals['server_domain'] + ":" + vals['service_options']['port']['hostport'] + "," + "db=" + vals['base_unique_name_'] + "," + "user=" + vals['apptype_admin_name'] + ", password=" + vals['base_admin_passwd'] + ")", context)
...
#            execute.execute(ssh, ['mkdir', '-p', '/base-backup/' + vals['saverepo_name'] + '/filestore'], context)
            execute.execute(ssh, ['cp', '-R', '/opt/odoo/' + vals['service_name'] + '/filestore/' + vals['base_unique_name_'], '/base-backup/' + vals['saverepo_name'] + '/filestore'], context)
            ssh.close()
            sftp.close()
        return
